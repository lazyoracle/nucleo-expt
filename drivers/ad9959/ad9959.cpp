...
AD9959::AD9959(SPI &spi_bus, const Pins &pins, uint32_t ref_freq, uint8_t mult)
    : ref_freq_{ref_freq}, mult_{mult}, spi_bus_{spi_bus},
      ...
      update_{pins.UpdatePin, 0}, sdio_3_{pins.SDIO_3, 0}, p0_{pins.Profile0Pin, 0},
...
void AD9959::reset_comm() { pulse(sdio_3_); }
...
void AD9959::set_amp(AD9959::Channel ch, uint32_t asf)
{
    /** 
    This function configures the ACR for manual amplitude control
    which is given by the 10 bit (1024 level) Amplitude Scale factor
    The mask value of 4096 corresponds to binary:
    000000000001000000000000 -> 24 bit ACR described below
    00000000000100 | 0..0 -> Values of ACR for manual control
    0..1..0 | 0000000000 -> Mask for ASF
    */
    
    set_csr(ch);
    uint32_t mask_acr = 4096;
    uint32_t acr_val = mask_acr | asf;

    write_register(AD9959::ACR, acr_val);
...
//Work in progress. Not fully implemented and tested yet
...
void AD9959::set_freq_linear_sweep_params(
  ...
  cfr_params.amplitude_frequency_select = 0b00; // Frequency sweep
  cfr_params.linear_sweep_enable = 0;
  cfr_params.linear_sweep_no_dwell = 0;
  cfr_params.load_srr_at_io_update = 0;
...
//Work in progress. Not fully tested and implemented yet.
...
void AD9959::set_csr(AD9959::Channel ch) {
  ...
  csr_val.channel = ch;
  csr_val.serial_mode_select = AD9959::IO2Wire;
